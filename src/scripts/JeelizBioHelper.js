...
import Worker from '../scripts/bio.worker.js';
...
const JeelizBioHelper = (function(){
    ...
    function initTrakking(){
        const worker = new Worker();

        worker.postMessage({ a: 12 });
        worker.onmessage = function (event) {
            console.log('pi es ', event.data);
            // const duplca = duplica(event.data.number);
            // console.log(duplca);
        };
        
        function duplica(event){
            return event*2
        }
    ...
    const app = {
        init: function(spec){
            ...
            JEEFACETRANSFERAPI.onLoad( () => {
                ...
                // console.log(CVD);
                JEEFACETRANSFERAPI.switch_displayVideo(true);
...
export default JeelizBioHelper;